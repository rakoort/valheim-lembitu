...
- PortalRules configuration is locked and the portal map is disabled. DataForge is locked, with its committed data files pinning an empty override state rather than silently accepting future package examples. Creature difficulty moved to CreatureLevelAndLootControl on 2026-09-17 (ADR-0015, #84): its `[2 - Creatures]` section holds base creature health at 200% and damage growth of 25% per star, and `[4 - Bosses]` carries boss health through pinned star chances, because that mod has no boss base-health key. CreatureManager's biome level preset, its committed `levels.yml` and its four sibling data files retired with it.
- Difficulty is fixed against headcount, and pinned that way four times over. Vanilla makes every creature tougher for each player standing nearby — 40% effective health and 4% damage each — so the same boss was a different fight depending on who logged in. CreatureLevelAndLootControl postfixes `Game.GetPlayerDifficulty`, `GetDifficultyDamageScaleEnemy` and `GetDifficultyDamageScalePlayer` rather than writing vanilla's fields, so the overlay zeroes both percentages, clamps the minimum and maximum nearby-player count to one, and zeroes the additional-player key. Any one of those left at stock is a lever that silently restores scaling.
...
- The 2026-09-16 review pinned six surfaces nobody had ever enforced (one of which, EpicMMOSystem's XP economy, left with the mod in #80), plus two loader-level ones. DiveIn's swimming block, SkadiNet's measured network profile and DynamicLocations' two spawn-point keys are pinned as they ran. Max Dungeon Rooms is 20–40 with its three per-dungeon overrides disabled, world-permanent under ADR-0009. MaxPlayerCount is 20, which is where the Roster's capacity is actually implemented. BepInEx itself gets `AppendLog = true`, so a restart stops destroying the log of the boot before it. DiscordConnector sends no positions and refuses `@here`/`@everyone`, with the eight lifecycle, join, leave, death and shout toggles pinned on; the webhook URL is a secret and stays in `config/launch/launch.secret.env`.
...
to kill, while bosses were fine, because bosses had their own smaller multiplier. That mod is gone
as of #84; the lesson is not, because its replacement also states affix chances per effect.
...
ordinary chances went to 0.125 each, 4 creatures in 100 — cut to 0.25 and halved again the same
day — while boss and Enforcer chances stayed high, so the named creature kept its signature. The
four-in-a-hundred figure is the decision that has to be re-expressed in the replacement mod's own
affix table (#84).

That gap closed by removal rather than by commitment: `karma.yml` was never in the overlay, and
Karma left the run with the mod on 2026-09-17 (ADR-0015).
...
- DataForge is for tuning, not cloned or custom items. Groups and Guilds are not fallback membership systems (`docs/adr/0004-two-power-curves.md:37-38`, `docs/adr/0008-clan-is-the-only-membership-authority.md:23-27`). CreatureManager's empty overrides used to sit beside that rule; the mod retired on 2026-09-17 and CreatureLevelAndLootControl replaced it with plain percentages (ADR-0015).
