...
| [VentureValheim/Progression](https://github.com/OrianaVenture/VentureValheim/blob/master/Progression/src/SkillsManager.cs) `SkillsManager.UpdateCache` at `master`, 2026-09-16 | The skill floor and ceiling are decided independently: an override wins for its own side only, so boss-key coupling can drive the floor while the ceiling stays overridden at 100. The package README's claim that the coupling "will only work if you DO NOT override the minimum and/or maximum" is imprecise. The floor is `BossKeysSkillPerKey × bossesDefeated`, counted from private keys when `UsePrivateKeys` is on | `config/enforced/com.orianaventure.mod.WorldAdvancementProgression.cfg`, `docs/rules.md` |
| [VentureValheim/Progression](https://github.com/OrianaVenture/VentureValheim/blob/master/Progression/README.md) README at `master` | The locks are material-scoped, not blanket: cooking, eating, building, equipping and crafting are gated per the biome of the item's materials, so a keyless character still uses Meadows-tier content | `docs/rules.md`, `issue://67` |
| Thunderstore Valheim package index (`https://thunderstore.io/c/valheim/api/v1/package/`, 11,269 packages, fetched 2026-09-16) | No published mod restricts where or how often a player changes PvP stance, and none scopes corpse access to the dead player's flag. Establishes that #67's behaviour is new rather than a reimplementation | `issue://67`, `docs/rules.md` |
| [sighsorry/FearNoSpear](https://thunderstore.io/c/valheim/p/sighsorry/FearNoSpear/) 1.0.8 readme | `OwnerOnlyTombstones` authorises tombstone access from the owner's stored player ID and grants the owner's active Clan roster, Guests included, through `sighsorry.Clan`'s API; unconditional on PvP state | `issue://67` |
| [VentureValheim/Venture_Multiplayer_Tweaks](https://thunderstore.io/c/valheim/p/VentureValheim/Venture_Multiplayer_Tweaks/) 1.0.0 readme | `OverridePlayerPVP` with `ForcePlayerPVPOn` disables the vanilla PvP toggle in the UI, with admin bypass; skill loss and teleport can be scoped to PvP deaths only | `issue://67` |
| [Tristan/Valheim_PvP_Tweaks](https://thunderstore.io/c/valheim/p/Tristan/Valheim_PvP_Tweaks/) 1.3.0 readme | A Battle status effect applied on aggression blocks teleporting for its duration — prior art for adding friction to stance and escape behaviour | `issue://67` |
| `EpicLoot.dll` 0.14.5, sha256 `902ac92b…c59bbb8` — the exact lock entry — decompiled 2026-09-16 with ilspycmd 11. Types read: `EpicLoot.Patching.{FilePatching,PatchFile,Patch,PatchAction,PatchReloadDebouncer}`, `EpicLoot.Config.{ELConfig,ConfigVersionManager,ConfigVersionState}`, `LootRoller.GetEffectCountsPerRarity`, and `Jotunn.Managers.SynchronizationManager` 2.30.0 | The `patches/` mechanism: patch files live in `BepInEx/config/EpicLoot/patches`, scanned recursively; the `PatchFile`/`Patch` schema; the eleven-member `PatchAction` enum, of which `Merge` and `MultiAdd` are undocumented upstream; `Require`/`RequireAll` turn a zero-match path from silent to loud; `ConfigVersionManager.RefreshUnmodifiedConfigs` short-circuits any patched domain, so hashes and declines never gate a patch; `baseconfig/*.json` is a generated artifact once patched; and `loottables.json` is pushed to every client at join, so the patch must be server-side | `issue://73`, `docs/wiki/pack.md` |
| `assembly_valheim.dll` 1.0.12, decompiled 2026-09-16 with ilspycmd. Types read: `Player`, `PlayerProfile`, `InventoryGui`, `Inventory`, `InventoryGrid`, `OfferingBowl`, `Vegvisir`, `BossStone`, `Character` | The stance's real storage and seams: `Player.SetPVP` writes `ZDOVars.s_pvp` on the player's own ZDO and `PlayerProfile` stores nothing, so the flag is off at every login; `Player.CanSwitchPVP()` returns `m_lastCombatTimer > 10f` and `InventoryGui` greys its `m_pvp` toggle from it, which is where a place predicate belongs; "boss runestone" names three distinct objects — `OfferingBowl` (the summoning altar, carries `m_bossPrefab`), `Vegvisir` (the stones that reveal a boss location) and `BossStone` (the power stones at the sacrificial stones) — so detection is by component, not a prefab list | `docs/adr/0013-the-stance-is-a-server-side-per-character-record.md`, `docs/rules.md`, `issue://67` |
| `PvPBiomeDominions.dll` 1.7.8 and `AzuExtendedPlayerInventory.dll` 2.4.14 — the exact lock entries — decompiled 2026-09-16 with ilspycmd | Retention is computed per item in a `Player.CreateTombStone` prefix that takes no killer: an item is kept only when `m_equipped` with keep-equipped on, or `m_gridPos.y == 0` with keep-hotbar on, so row 0 is the hotbar and nothing else. AzuEPI's `Inventory.MoveInventoryToGrave` prefix raises the grave's height to the full inventory height, so equipment-slot and quick-slot items reach the tombstone | `docs/rules.md`, `issue://67` |
| `Clan.dll` 1.0.10 and `STUWard.dll` 1.3.15 — the exact lock entries — decompiled 2026-09-16 with ilspycmd | The membership and claim surfaces ADR-0008 requires: `ClanApi.ResolveMemberships` and `ClanApi.ResolveWardAuthorization` answer primary and Guest membership from a platform id and a character player id, and `WardAccessApi.IsManagedWard` / `TryCheckContainerAccess` answer the ward claim, so neither has to be inferred by comparing identifiers | `docs/adr/0013-the-stance-is-a-server-side-per-character-record.md`, `issue://67` |
| `AzuCraftyBoxes.dll` 1.8.19 (sha256 `5a191f9c…3084d`), `AzuHoverStats.dll` 1.1.10 (`56db0b8d…a2625b`) and `ProximityVoiceChat.dll` 1.0.2 (`c45a31e4…49ec99`) — the exact lock entries — decompiled 2026-09-16 with ilspycmd 11 | Which of the five adopted quality-of-life mods the server can enforce, and which refuse a client. AzuCraftyBoxes and AzuHoverStats each patch `ZNet.OnNewConnection` to invoke `<Mod>_VersionCheck` and `ZNet.RPC_PeerInfo` to disconnect a peer absent from `ValidatedPeers`, which is what makes a server-side install mandatory for players — not `ConfigSync.ModRequired`, which both leave false. AzuHoverStats has no `ConfigSync` at all: every entry is a plain `Config.Bind`, so nothing in it is server-synced. Its chest readout is access-checked, because the `Container.GetHoverText` postfix bails on `m_checkGuardStone && !PrivateArea.CheckAccess(...)` and `STUWard.dll` 1.3.15 prefixes that same method with clan-resolved trust. ProximityVoiceChat's synced keys are exactly `[2 - Proximity]` and three of `[4 - Codec]`; microphone, playback, indicator and keybind entries are all `synchronizedSetting: false`. AzuCraftyBoxes reads its restriction file through `YamlUtils.ReadYaml`, which turns a blank file into an empty dictionary, so an empty `Azumatt.AzuCraftyBoxes.yml` leaves everything in range pullable | `config/enforced/Azumatt.AzuCraftyBoxes.cfg`, `config/enforced/Azumatt.ProximityVoiceChat.cfg`, `docs/modstack.md`, `docs/wiki/pack.md`, `issue://78` |
| `AzuExtendedPlayerInventory.dll` 2.4.14, sha256 `852dcde0…6840b6`, decompiled 2026-09-16 with ilspycmd 11 | AzuEPI does ship favouriting: `[10 - Favoriting]` binds `Favoriting Modifier Key` (Left Shift) plus border colours and tooltip text, all `synchronizedSetting: false`. Corrects the claim in `docs/rules.md` and `docs/wiki/pack.md` that favourites had no counterpart after the v7 swap | `docs/rules.md`, `docs/wiki/pack.md`, `issue://78` |
