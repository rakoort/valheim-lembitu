...
public class ConfigSync
{
	...
	internal static class RegisterRPCPatch
	{
		...
		private static void Postfix(ZNet __instance)
		{
			...
			{
				MethodInfo? listContainsId = AccessTools.DeclaredMethod(typeof(ZNet), nameof(ZNet.ListContainsId));
				SyncedList adminList = (SyncedList)AccessTools.DeclaredField(typeof(ZNet), nameof(ZNet.m_adminList)).GetValue(ZNet.instance);
	...
	private IEnumerator sendZPackage(long target, ZPackage package)
	{
		...
		List<ZNetPeer> peers = (List<ZNetPeer>)AccessTools.DeclaredField(typeof(ZRoutedRpc), nameof(ZRoutedRpc.m_peers)).GetValue(ZRoutedRpc.instance);
	...
	private class SendConfigsAfterLogin
	{
		...
		private static void Prefix(ref Dictionary<Assembly, BufferingSocket>? __state, ZNet __instance, ZRpc rpc)
		{
			if (__instance.IsServer())
			{
				...
				if (AccessTools.DeclaredMethod(typeof(ZNet), nameof(ZNet.GetPeer), new[] { typeof(ZRpc) }).Invoke(__instance, new object[] { rpc }) is ZNetPeer peer && ZNet.m_onlineBackend != OnlineBackendType.Steamworks)
				{
					FieldInfo field = AccessTools.DeclaredField(typeof(ZNetPeer), nameof(ZNetPeer.m_socket));
					if (field.GetValue(peer) is ZPlayFabSocket playFabSocket)
					{
						typeof(ZPlayFabSocket).GetField(nameof(ZPlayFabSocket.m_remotePlayerId)).SetValue(bufferingSocket, playFabSocket.m_remotePlayerId);
					}
		...
		private static void Postfix(Dictionary<Assembly, BufferingSocket> __state, ZNet __instance, ZRpc rpc)
		{
			...
			void SendBufferedData()
			{
				if (rpc.GetSocket() is BufferingSocket bufferingSocket)
				{
					...
					if (AccessTools.DeclaredMethod(typeof(ZNet), nameof(ZNet.GetPeer), new[] { typeof(ZRpc) }).Invoke(__instance, new object[] { rpc }) is ZNetPeer peer)
					{
						AccessTools.DeclaredField(typeof(ZNetPeer), nameof(ZNetPeer.m_socket)).SetValue(peer, bufferingSocket.Original);
					}
				}
			...
			if (AccessTools.DeclaredMethod(typeof(ZNet), nameof(ZNet.GetPeer), new[] { typeof(ZRpc) }).Invoke(__instance, new object[] { rpc }) is not ZNetPeer peer)
			{
				SendBufferedData();
				return;
			}
...
public class VersionCheck
{
	...
	{
		if (PatchProcessor.GetPatchInfo(AccessTools.DeclaredMethod(typeof(ZNet), nameof(ZNet.Awake)))?.Postfixes.Count(p => p.PatchMethod.DeclaringType == typeof(ConfigSync.RegisterRPCPatch)) > 0)
		{
			return;
		}
	...
	static VersionCheck()
	{
		typeof(ThreadingHelper).GetMethod(nameof(ThreadingHelper.StartSyncInvoke))!.Invoke(ThreadingHelper.Instance, new object[] { (Action)PatchServerSync });
	}
	...
	private static void RegisterAndCheckVersion(ZNetPeer peer, ZNet __instance)
	{
		...
		IDictionary rpcFunctions = (IDictionary)typeof(ZRpc).GetField(nameof(ZRpc.m_functions), BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(peer.m_rpc);
