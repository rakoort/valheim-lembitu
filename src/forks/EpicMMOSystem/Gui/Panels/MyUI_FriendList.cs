        ...
        public FriendsCell(GameObject cell, FriendInfo friend, ZNet.PlayerInfo info, StatusFriend status)
        {
            ...
            switch (status)
            {
                case StatusFriend.online:
                    ...
                    // The "add to group" button invited a friend into a Smoothbrain Groups group.
                    // Groups is cut: Clan is the only membership authority (ADR-0008), so the
                    // button is hidden for every friend, online or not.
