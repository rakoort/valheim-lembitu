...
{
    public const string PluginGuid = "lembitu.hello";
    public const string PluginName = "Lembitu.Hello";
    ...
    {
        Logger.LogInfo(
            $"{PluginName} {PluginVersion} loaded on {ValheimVersion.GetVersionString(false)} "
            + $"(built against network version {CompiledNetworkVersion})");

        uint? live = LiveNetworkVersion();
        if (live is null)
        {
            Logger.LogWarning(
        ...
        else if (live != CompiledNetworkVersion)
        {
            Logger.LogError(
